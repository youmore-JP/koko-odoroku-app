...
- 報告書の保存時(`saveIncident`→`applyIncidentToSupport`)に当事者利用者の記録を更新: 無ければ新規作成 / **自動生成フラグ`auto: true`の記録**なら報告内容に置き換え(置き換えたら`auto`は外れる) / 手書き・追記済み(`auto`なし)なら末尾に追記。報告書の削除では記録を変更しない。`auto`フラグは `ensureSupportRecord()` と一括作成時に付与され、`saveSupportForm()` の新規保存では付かない(旧バージョンの自動生成記録には無いため、以後は追記扱いになる)
